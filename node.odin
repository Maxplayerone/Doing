...
add_element :: proc(node: ^Node, name: string) {
	...
	element_rect_with_outline := rl.Rectangle {
		node.body_rect.x,
		node.body_rect.y + node.horizontal_cursor,
		node.body_rect.width,
		ElementHeight,
	}
...
add_batch :: proc(node: ^Node, name: string) {
	batch: Batch
	batch.name = name
	batch.rect_with_outline = rl.Rectangle {
		node.body_rect.x,
		node.body_rect.y + node.horizontal_cursor,
		node.body_rect.width,
		BatchHeight,
	...
	batch.text_rect, _ = split_rect(batch.rect, 0.5)
...
node_render :: proc(node: Node) {
	...
	for batch in node.batches {
		rl.DrawRectangleRec(batch.rect, rl.ORANGE)
		padding := rl.Vector2{10.0, 10.0}
		adjust_and_draw_text(batch.name, batch.text_rect, padding, 30.0)
	}
